...
plot_cells(monocle.obj, 
           ...
           cell_size = 1, 
           color_cells_by = "cca.cluster.0.5", 
...
## Pseudotime on UMAP with cell cca.cluster.0.5 {.tabset}
...
for (selected.starting.point in list_of_principal_points){
  ...
  for (input.gene in gt0.5_genes){
    ...
    p2 <- plot_genes_in_pseudotime(color_cells_by="cca.cluster.0.5", cds_subset = subset_monocle.obj, min_expr = 0.5, horizontal_jitter = 0.1)
    p <- p1 + p2
    ggsave(plot = p, 
