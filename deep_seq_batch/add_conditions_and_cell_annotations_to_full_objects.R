...
for (PROJECT in all.PROJECTS){
  ...
  if (file.exists(file.path(path.to.01.output, sprintf("Project_%s.addedCellAnnotation.rds", PROJECT))) == FALSE){
    s.obj <- readRDS(file.path(path.to.01.output, sprintf("Project_%s.rds", PROJECT)))
    
    if (PROJECT %in% c("SBharadwaj_20240318_Sample_1_4_7_8", 
                       "SBharadwaj_20240318_Sample_1_4_7_8_2_5")){
      ##### add Cell annotation
    ...
    ##### add conditions and Sample type
    ...
    print(sprintf("File %s exists", file.path(path.to.01.output, sprintf("Project_%s.addedCellAnnotation.rds", PROJECT))))
  }
}
