...
# cont.sub.cluster.idx <- "DC"

path.to.main.input <- file.path(outdir, "SeuratV5", PROJECT)
path.to.main.output <- file.path(path.to.main.input, "data_analysis")
path.to.07.output <- file.path(path.to.main.output, "07_output", sub.cluster.idx)
dir.create(path.to.07.output, showWarnings = FALSE, recursive = TRUE)

s.obj <- readRDS(file.path(path.to.07.output, cont.sub.cluster.idx, "s8_output", sprintf("%s.output.s8.rds", PROJECT)))
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
if (file.exists(file.path(path.to.07.output,sprintf("Project_%s_%s.rds", PROJECT, sub.cluster.idx))) == FALSE){
  saveRDS(s.obj, file.path(path.to.07.output,sprintf("Project_%s_%s.rds", PROJECT, sub.cluster.idx)))  
} else {
  s.obj <- readRDS(file.path(path.to.07.output,sprintf("Project_%s_%s.rds", PROJECT, sub.cluster.idx)))
}
```
