...
path.to.s.obj <- "/media/hieunguyen/HD01/outdir/CRC1382/SBharadwaj_20240318/SBharadwaj_20240318_Sample_1_4_7_8_2_5/data_analysis/01_output/Project_SBharadwaj_20240318_Sample_1_4_7_8_2_5.addedConditions.rds"
path.to.save.output <- "/media/hieunguyen/HD01/outdir/CRC1382/SBharadwaj_20240318/SBharadwaj_20240318_Sample_1_4_7_8_2_5/data_analysis/04_output/SBharadwaj_20240318_Sample_1_4_7_8_2_5/full"
...
monocle.obj <- new_cell_data_set(
  expression_data = GetAssayData(s.obj, slot = "counts"),
...
DimPlot(object = s.obj, 
        ...
        label.box = TRUE, 
        repel = TRUE, 
        group.by = "cca.cluster.0.5", 
        label.size = 8, 
        pt.size = 1.5) + 
  ggtitle("UMAP: original Seurat object with cell type cca.cluster.0.5s")
```
...
get_earliest_principal_node <- function(cds, cluster.id){
  cell_ids <- which(colData(cds)[, "cca.cluster.0.5"] == cluster.id)
  
  closest_vertex <-
    cds@principal_graph_aux[["cca_UMAP"]]$pr_graph_cell_proj_closest_vertex
  closest_vertex <- as.matrix(closest_vertex[colnames(cds), ])
  root_pr_nodes <-
    igraph::V(principal_graph(cds)[["cca_UMAP"]])$name[as.numeric(names(which.max(table(closest_vertex[cell_ids,]))))]
  root_pr_nodes
}
```
...
for (principal_node in list_of_principal_points){
  monocle.obj <- order_cells(cds = monocle.obj, reduction_method = "cca_UMAP", root_pr_nodes = principal_node)
  s.obj <- AddMetaData(
  object = s.obj,
...
for (selected.starting.point in list_of_principal_points){
  ...
  for (case in names(split.selected.genes)){
    ...
    subset_monocle.obj <- order_cells(cds = subset_monocle.obj, reduction_method = "cca_UMAP", root_pr_nodes = selected.starting.point)
...
closest_vertex <- monocle.obj@principal_graph_aux[["cca_UMAP"]]$pr_graph_cell_proj_closest_vertex
...
variable.genes <- VariableFeatures(s.obj)
if (file.exists(file.path(path.to.save.output, "sce.rds")) == FALSE){
  sce <- fitGAM(counts = input.GAM.counts,
              ...
              nknots = 5,
              parallel = FALSE, 
              genes = intersect(row.names(input.GAM.counts), variable.genes),
              BPPARAM = BPPARAM)
