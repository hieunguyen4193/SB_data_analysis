...
# path.to.save.output <-  "/media/hieunguyen/HD01/outdir/CRC1382/SBharadwaj_20240318/SBharadwaj_20240318_Sample_1_4_7_8_2_5/data_analysis_with_cell_annotations/02_output/full/ctrl_gut_myeloid_vs_treated_gut_myeloid"

dir.create(path.to.save.output, showWarnings = FALSE, recursive = TRUE)

library(clusterProfiler)
if (packageVersion("org.Mm.eg.db") != "3.20.0"){
  install.packages("https://bioconductor.org/packages/release/bioc/src/contrib/AnnotationDbi_1.68.0.tar.gz", 
                   type = "source", repos = NULL)
  install.packages("https://bioconductor.org/packages/release/data/annotation/src/contrib/org.Mm.eg.db_3.20.0.tar.gz", 
                   type = "source", repos = NULL)
}
library(org.Mm.eg.db)
```
...
if (file.exists(file.path(path.to.save.output, savename)) == FALSE){
  de.markers <- hash()
  de.markers.raw <- hash()
  num.clusters <- length(unique(s.obj$cell.annotation))
  
  for (cluster.id in unique(s.obj$cell.annotation)){
    tmp.s.obj <- subset(s.obj, cell.annotation == cluster.id)
...
for (cluster.id in all.clusters.in.de.markers){
  ...
  if ("data" %in% colnames(inputdf) | nrow(inputdf) == 0){
    ...
    if (length(c(top10up.genes$Gene, top10down.genes$Gene)) > 1){
      p <- DoHeatmap(object = subset(s.obj, cell.annotation == str_replace(cluster.id, "cluster_", "")), 
