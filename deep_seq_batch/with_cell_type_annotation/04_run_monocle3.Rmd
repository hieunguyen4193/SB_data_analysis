...
if (file.exists(file.path(path.to.save.output, "monocle3.rds")) == FALSE){
  ...
  monocle.obj@int_colData@listData$reducedDims$UMAP <- s.obj@reductions$cca_UMAP@cell.embeddings
  #####----------------------------------------------------------------------#####
...
plot_cells(monocle.obj, 
           ...
           cell_size = 1, 
           color_cells_by = "cell.annotation", 
...
## Pseudotime on UMAP with cell cell.annotation {.tabset}

```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=18}
for (principal_node in list_of_principal_points){
  Idents(s.obj) <- "cell.annotation"
  cat(sprintf("#### Principal node %s \n", principal_node))
  p <- FeaturePlot(object = s.obj, reduction = "cca_UMAP", features = sprintf("pseudotime_%s", principal_node), label = TRUE, label.size = 8, label.color = "red") + scale_color_viridis_c()
  print(p)
  cat("\n \n")
}
```
...
for (selected.starting.point in list_of_principal_points){
  ...
  for (input.gene in gt0.5_genes){
    ...
    p2 <- plot_genes_in_pseudotime(color_cells_by="cell.annotation", cds_subset = subset_monocle.obj, min_expr = 0.5, horizontal_jitter = 0.1)
    p <- p1 + p2
    ggsave(plot = p, 
...
for (tmp.col in colnames(cellWeights)){
  p <- DimPlot(object = s.obj, reduction = "cca_UMAP", group.by = tmp.col, label = TRUE, pt.size = 1.5)
  print(p)
}
```
