---
title: "Data analysis for sub-clustering data, Project `r params$PROJECT`, sub-clustering version `r params$sub.cluster.idx`, RE-INTEGRATION `params$integration`"
author:
...
path.to.06.output <- file.path(path.to.main.output, "06_output", "preprocess_sub_clusters", sub.cluster.idx)
dir.create(path.to.06.output, showWarnings = FALSE, recursive = TRUE)
path.to.save.subclusters <- file.path(path.to.main.output, "06_output", "raw_sub_clusters", sub.cluster.idx)
```


```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=18}
if (integration == TRUE){
  print("USING RE-INTEGRATED DATA AFTER SUB-CLUSTERING")
  ...
  path.to.06.output <- file.path(path.to.06.output, "ReIntegration")
} else {
  print("NO RE-INTEGRATED DATA AFTER SUB-CLUSTERING")
  s.obj <- readRDS(file.path(path.to.save.subclusters, "s8_output", sprintf("%s.noIntegration.rds", PROJECT)))
  reduction.name <- "SCT_UMAP"
  cluster.name <- "seurat_clusters"
  path.to.06.output <- file.path(path.to.06.output, "noReIntegration")
}
dir.create(path.to.06.output, showWarnings = FALSE, recursive = TRUE)
```
