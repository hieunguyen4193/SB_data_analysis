...
if (integration == TRUE){
 path.to.s.obj <- file.path(path.to.07.output, 
                           "preprocees_sub_clusters", 
                           "v0.1", 
                           "DC", 
                           "ReIntegration", 
                           "Project_SBharadwaj_20240318_Sample_1_4_7_8_2_5_v0.1_DC.rds") 
 reduction.name <- "cca_UMAP"
 cluster.name <- "cca.cluster.0.5"
 path.to.01.output <- file.path(path.to.01.output, "ReIntegration")
} else {
  path.to.s.obj <- file.path(path.to.07.output, 
                           "preprocees_sub_clusters", 
                           "v0.1", 
  ...
  path.to.01.output <- file.path(path.to.01.output, "noReIntegration")
}
...
DimPlot(object = s.obj, reduction = reduction.name, cells.highlight = keep.cells, label = TRUE, label.box = TRUE, repel = TRUE) +
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=18}
DimPlot(object = s.obj, reduction = reduction.name, cells.highlight = remove.cells, label = TRUE, label.box = TRUE, repel = TRUE) +
  theme(legend.position = "none") + 
...
dir.create(file.path(path.to.01.output, "s8_output"), showWarnings = FALSE, recursive = TRUE)
if (integration == FALSE){
  if (file.exists(file.path(path.to.01.output, "s8_output", sprintf("%s_DCs_Cd3g.output.s8.rds", PROJECT))) == FALSE){
    ...
    saveRDS(s.obj, file.path(path.to.01.output, "s8_output", sprintf("%s_DCs_Cd3g.output.s8.rds", PROJECT)))    
  } else {
