...
# path.to.s.obj <- "/media/hieunguyen/HD01/outdir/CRC1382/SBharadwaj_20250102/SBharadwaj_20240318_Sample_1_4_7_8_2_5/data_analysis/01_output/custom_output/DCs_Cd3g/noReIntegration/Project_SBharadwaj_20240318_Sample_1_4_7_8_2_5.addedInfo.rds"
# sample1 <- "treated_gut_myeloid"
# path.to.cellchat1 <- "/media/hieunguyen/HD01/outdir/CRC1382/SBharadwaj_20250102/SBharadwaj_20240318_Sample_1_4_7_8_2_5/data_analysis/03_output/DCs_non_Cd3g_exprs/seurat_clusters/condition/treated_gut_myeloid/CellChat_object.treated_gut_myeloid.Filter10.rds"
# sample2 <- "ctrl_gut_myeloid"
# path.to.cellchat2 <- "/media/hieunguyen/HD01/outdir/CRC1382/SBharadwaj_20250102/SBharadwaj_20240318_Sample_1_4_7_8_2_5/data_analysis/03_output/DCs_non_Cd3g_exprs/seurat_clusters/condition/ctrl_gut_myeloid/CellChat_object.ctrl_gut_myeloid.Filter10.rds"
# path.to.save.output <- "/media/hieunguyen/HD01/outdir/CRC1382/SBharadwaj_20250102/SBharadwaj_20240318_Sample_1_4_7_8_2_5/data_analysis/03_output/DCs_non_Cd3g_exprs/seurat_clusters/condition/treated_gut_myeloid_vs_ctrl_gut_myeloid"
...
## Group by conditions
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
DimPlot(object = s.obj, reduction = reduction.name, label = TRUE, label.box = TRUE, repel = TRUE, group.by = condition.name)
```
...
# https://github.com/jinworks/CellChat/issues/196

netAnalysis_signalingRole_heatmap.fix <- function (object, signaling = NULL, pattern = c("outgoing", 
...
ht1 = netAnalysis_signalingRole_heatmap.fix(object.list[[i]], pattern = "outgoing", signaling = pathway.union, title = names(object.list)[i], width = 21, height = 15)
ht2 = netAnalysis_signalingRole_heatmap.fix(object.list[[i+1]], pattern = "outgoing", signaling = pathway.union, title = names(object.list)[i+1], width = 21, height = 15)
...
for (source.cluster in plt.clusters){
  ...
  if (num.NA.ligand == nrow(source.net.down) | 
      num.NA.receptor == nrow(source.net.down) | 
      length(intersect(source.net.down$pathway_name, all.pathways)) == 0){
    gg1 <- ggplot() + ggtitle("No significant signaling signal to show!")
  } else {
...
# subset(net, 
#        net$interaction_name %in% pairLR.use.down$interaction_name &
#          net$source == source.cluster &
#          net$ligand.pvalues <= 0.05 &
#          net$receptor.pvalues <= 0.05)
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
source("/home/hieunguyen/CRC1382/src_2023/SBharadwaj/deep_seq_batch/with_cell_type_annotation/plot_gene_expression_from_Cellchat.R")
for (group in names(groups)){
  cat(sprintf("## Group %s {.tabset} \n \n", group))
  cat("\n \n")
  for (pathway in groups[[group]]){
