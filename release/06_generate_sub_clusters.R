...
# /home/hieunguyen/CRC1382/src_2023/SBharadwaj/release/sub_clustering_indices.R
...
for (PROJECT in c("SBharadwaj_20240318_Sample_3_6",
  ...
  path.to.save.subclusters <- file.path(path.to.06.output, "raw_sub_clusters", sub.cluster.idx)
  dir.create(file.path(path.to.save.subclusters, "s8_output"), showWarnings = FALSE, recursive = TRUE)
  
  if (file.exists(file.path(path.to.save.subclusters, 
                            "s8_output",
                            sprintf("%s.noIntegration.rds", PROJECT))) == FALSE){
    ...
    saveRDS(s.obj.no.integrated, file.path(path.to.save.subclusters, 
                                           "s8_output",
                                           sprintf("%s.noIntegration.rds", PROJECT)))
  } else {
    print(sprintf("File %s exists", file.path(path.to.save.subclusters, 
                                              "s8_output",
                                              sprintf("%s.noIntegration.rds", PROJECT))))
  }
  ...
  if (file.exists(file.path(path.to.save.subclusters, "s8_output", sprintf("%s.output.s8.rds", PROJECT))) == FALSE){
    ...
    s.obj.integrated <- s8.integration.and.clustering_V5(s.obj = s.obj, 
                                                         save.RDS.s8 = TRUE,
                                                         path.to.output = file.path(path.to.save.subclusters),
                                                         use.sctransform = TRUE,
                                                         num.PCA = num.PCA,
    ...
    print(sprintf("File %s exists", file.path(path.to.save.subclusters, "s8_output", sprintf("%s.output.s8.rds", PROJECT))))
  }
}
