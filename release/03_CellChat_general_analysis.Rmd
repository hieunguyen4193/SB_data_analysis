...
# path.to.s.obj <- "/media/hieunguyen/HD01/outdir/CRC1382/SBharadwaj_20250102/SBharadwaj_20240318_Sample_1_4_7_8_2_5/data_analysis/06_output/preprocees_sub_clusters/v0.1/noReIntegration/Project_SBharadwaj_20240318_Sample_1_4_7_8_2_5_v0.1.addedInfo.rds"
# path.to.save.output <- "/media/hieunguyen/HD01/outdir/CRC1382/SBharadwaj_20250102/SBharadwaj_20240318_Sample_1_4_7_8_2_5/data_analysis/03_output/06_output_v0.1/seurat_clusters/name/treated_gut_myeloid"
...
DimPlot(object = s.obj.original, reduction = reduction.name, label = TRUE, label.box = TRUE, repel = TRUE)
```
...
if (cluster.name %in% c("cca.cluster.0.5", "seurat_clusters")){
  ...
  rownames_to_column("barcode") %>%
  rowwise() %>%
  mutate(cluster = sprintf("cluster_%s", cell.annotation)) %>%
  column_to_rownames("barcode")
  num.cluster <- length(unique(s.obj$cell.annotation))
  all.cluster.idx <- to_vec (for (cluster.id in unique(s.obj$cell.annotation)) str_replace(cluster.id, "cluster_", "")) 
  meta.data$cluster <- factor(meta.data$cluster, levels = to_vec ( for (i in sort(all.cluster.idx)) sprintf("cluster_%s", i) ))
}
