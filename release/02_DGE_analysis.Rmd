...
for (cluster.id in all.clusters.in.de.markers){
  ...
  if ("data" %in% colnames(inputdf) | nrow(inputdf) == 0){
    ...
    to.plot.heatmap.genes <- c(top10up.genes$Gene, top10down.genes$Gene)
    sct.scale.genes <- row.names(GetAssayData(s.obj, slot = "scale.data", assay = "SCT"))
    if (length(intersect(to.plot.heatmap.genes, sct.scale.genes)) > 1){
      if (cluster.name == "cca.cluster.0.5"){
          p <- DoHeatmap(object = subset(s.obj, cca.cluster.0.5 == as.numeric(str_replace(cluster.id, "cluster_", ""))), 
                     features = to.plot.heatmap.genes, 
                     group.by = condition.name, 
                     assay = "SCT") + 
