...
for (row_i in seq(1, nrow(samplesheet))){
  ...
  if (file.exists(str_replace(path.to.s.obj, ".rds", ".addedInfo.rds")) == FALSE){
    ...
    meta.data <- s.obj@meta.data %>% rownames_to_column("barcode") %>% rowwise() %>%
      ...
      mutate(globa.condition = str_split(condition, "_")[[1]][[1]]) %>%
      column_to_rownames("barcode")
    
    meta.data <- meta.data[row.names(s.obj@meta.data), ]
    
    s.obj <- AddMetaData(object = s.obj, col.name = "condition", metadata = meta.data$condition)
    s.obj <- AddMetaData(object = s.obj, col.name = "globa.condition", metadata = meta.data$globa.condition)
