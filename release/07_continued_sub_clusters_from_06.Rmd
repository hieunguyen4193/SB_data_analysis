---
title: "Data analysis for sub-clustering data, Project `r params$PROJECT`, sub-clustering version `r params$sub.cluster.idx`, `r params$cont.sub.cluster.idx`, RE-INTEGRATION `r params$integration`"
author:
...
cont.sub.cluster.idx <- params$cont.sub.cluster.idx
integration <- params$integration
output.number <- params$output.number

path.to.main.input <- file.path(outdir, PROJECT)
path.to.main.output <- file.path(path.to.main.input, "data_analysis")

path.to.save.output <- file.path(path.to.main.output, output.number, "preprocess_sub_clusters", sub.cluster.idx, cont.sub.cluster.idx)
dir.create(path.to.save.output, showWarnings = FALSE, recursive = TRUE)
path.to.save.subclusters <- file.path(path.to.main.output, output.number, "raw_sub_clusters", sub.cluster.idx, cont.sub.cluster.idx)

if (integration == TRUE){
  print("reading integrated data")
  s.obj <- readRDS(file.path(path.to.save.subclusters, "s8_output", sprintf("%s_%s.output.s8.rds", PROJECT, cont.sub.cluster.idx)))  
  reduction.name <- "cca_UMAP"
  cluster.name <- "cca.cluster.0.5"
  path.to.save.output <- file.path(path.to.save.output, "ReIntegration")
} else {
  print("reading un-integrated data")
  s.obj <- readRDS(file.path(path.to.save.subclusters, "s8_output", sprintf("%s_%s.noIntegration.rds", PROJECT, cont.sub.cluster.idx)))
  reduction.name <- "SCT_UMAP"
  cluster.name <- "seurat_clusters"
  path.to.save.output <- file.path(path.to.save.output, "noReIntegration")
}
dir.create(path.to.save.output, showWarnings = FALSE, recursive = TRUE)
```
...
DimPlot(object = s.obj, reduction = reduction.name, label = TRUE, label.box = TRUE, label.size = 10, pt.size = 1, repel = TRUE, group.by = cluster.name) + 
...
## UMAP: all samples
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
DimPlot(object = s.obj, reduction = reduction.name, label = TRUE, label.box = TRUE, label.size = 10, pt.size = 1, repel = TRUE, group.by = "name") + 
...
for (cluster.id in sort(unique(cluster.markers$cluster))){
  ...
  p <- FeaturePlot(object = s.obj, reduction = reduction.name, features = head(tmp.cluster.markers, 9)$gene, ncol = 3, label = TRUE, pt.size = 0.5, label.size = 5, label.color = "red", slot = "data")  
  print(p)
  cat("\n \n")
}
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
all.cluster.ids <- sort(unique(s.obj@meta.data[[cluster.name]]))
...
cell.count.in.clusters <- table(s.obj$name, s.obj@meta.data[[cluster.name]]) %>% as.data.frame() %>%
...
DimPlot(object = s.obj, reduction = reduction.name, label.box = TRUE, label = TRUE, repel = TRUE, group.by = "prediction2")
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
FeaturePlot(s.obj, features = c("Ciita", "Foxp3", "Klrg1", "Gata3"), label = TRUE, reduction = reduction.name)
```
...
for (sample.id in unique(s.obj$name)){
  ...
  p <- FeaturePlot(subset(s.obj, name == sample.id), features = c("Ciita", "Foxp3", "Klrg1", "Gata3"), label = TRUE, reduction = reduction.name)  
  print(p)
  cat("\n \n")
}
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
VlnPlot(object = s.obj, features = c("Ciita", "Foxp3", "Klrg1", "Gata3"), group.by = cluster.name, ncol = 2, assay = "SCT", slot = "data")
```
...
for (sample.id in unique(s.obj$name)){
  cat(sprintf("### Sample %s \n", sample.id))
  p <- VlnPlot(object = subset(s.obj, name == sample.id), features = c("Ciita", "Foxp3", "Klrg1", "Gata3"), group.by = cluster.name, ncol = 2, assay = "SCT", slot = "data")  
  print(p)
  cat("\n \n")
}
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
if (file.exists(file.path(path.to.save.output,sprintf("Project_%s_%s_%s.rds", PROJECT, sub.cluster.idx, cont.sub.cluster.idx))) == FALSE){
  saveRDS(s.obj, file.path(path.to.save.output,sprintf("Project_%s_%s_%s.rds", PROJECT, sub.cluster.idx, cont.sub.cluster.idx)))  
} else {
  s.obj <- readRDS(file.path(path.to.save.output,sprintf("Project_%s_%s_%s.rds", PROJECT, sub.cluster.idx, cont.sub.cluster.idx)))
}
```
...
# }
# 
# if (file.exists(file.path(path.to.save.output, sprintf("%s_%s_%s", 
#                                                      PROJECT,
#                                                      sub.cluster.idx,
#                                                      cont.sub.cluster.idx))) == FALSE){
#   library(loupeR)
#   loupeR::setup()
#   create_loupe_from_seurat(
#       s.obj,
#       output_dir = file.path(path.to.save.output),
#       output_name = sprintf("%s_%s_%s", 
#                             PROJECT, 
#                             sub.cluster.idx,
#                             cont.sub.cluster.idx),
