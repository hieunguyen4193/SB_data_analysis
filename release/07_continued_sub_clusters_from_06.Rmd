...
path.to.07.output <- file.path(path.to.main.output, "07_output", "preprocees_sub_clusters", sub.cluster.idx, cont.sub.cluster.idx)
dir.create(path.to.07.output, showWarnings = FALSE, recursive = TRUE)
path.to.save.subclusters <- file.path(path.to.main.output, "07_output", "raw_sub_clusters", sub.cluster.idx, cont.sub.cluster.idx)
...
if (integration == TRUE){
  ...
  path.to.07.output <- file.path(path.to.07.output, "ReIntegration")
} else {
  ...
  path.to.07.output <- file.path(path.to.07.output, "noReIntegration")
}
dir.create(path.to.07.output, showWarnings = FALSE, recursive = TRUE)
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
if (file.exists(file.path(path.to.07.output,sprintf("Project_%s_%s_%s.rds", PROJECT, sub.cluster.idx, cont.sub.cluster.idx))) == FALSE){
  saveRDS(s.obj, file.path(path.to.07.output,sprintf("Project_%s_%s_%s.rds", PROJECT, sub.cluster.idx, cont.sub.cluster.idx)))  
} else {
  s.obj <- readRDS(file.path(path.to.07.output,sprintf("Project_%s_%s_%s.rds", PROJECT, sub.cluster.idx, cont.sub.cluster.idx)))
}
```
...
if (file.exists(file.path(path.to.07.output, sprintf("%s_%s_%s", 
                                                     PROJECT,
                                                     sub.cluster.idx,
